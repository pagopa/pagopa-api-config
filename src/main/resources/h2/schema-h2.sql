...
create table NODO4_CFG.IBAN_ATTRIBUTES
(
    OBJ_ID                numeric GENERATED BY DEFAULT AS IDENTITY,
    ATTRIBUTE_NAME        varchar(16) not null,
    ATTRIBUTE_DESCRIPTION varchar(32) not null,
    constraint PK_IBAN_ATTRIBUTES
        primary key (OBJ_ID)
);
...
(
    ...
    FK_ATTRIBUTE_MASTER numeric not null,
    constraint PK_IBAN_ATTRIBUTES_MASTER
        primary key (OBJ_ID),
    constraint FK_IBAN_MASTER
        foreign key (FK_IBAN_MASTER)
            references NODO4_CFG.IBAN_MASTER,
    constraint FK_ATTRIBUTE_MASTER
            foreign key (FK_ATTRIBUTE_MASTER)
