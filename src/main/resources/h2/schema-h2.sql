create sequence NODO4_CFG.HIBERNATE_SEQUENCE START WITH 100 INCREMENT BY 1;
...
create table NODO4_CFG.IBAN_ATTRIBUTE
(
    OBJ_ID                numeric GENERATED BY DEFAULT AS IDENTITY,
    ATTRIBUTE_NAME        varchar(16) not null,
    ATTRIBUTE_DESCRIPTION varchar(32),
    constraint PK_IBAN_ATTRIBUTE
        primary key (OBJ_ID)
);
create table NODO4_CFG.ICA_BINARY_FILE
(
    OBJ_ID        numeric GENERATED BY DEFAULT AS IDENTITY,
    FILE_CONTENT  blob,
    FILE_HASH     blob,
    FILE_SIZE     numeric not null,
    constraint PK_ICA_BINARY_FILE
        primary key (OBJ_ID)
);
create table NODO4_CFG.IBAN_MASTER
(
    OBJ_ID              numeric GENERATED BY DEFAULT AS IDENTITY,
...
create table NODO4_CFG.IBAN_ATTRIBUTE_MASTER
(
    OBJ_ID              numeric GENERATED BY DEFAULT AS IDENTITY,
    FK_IBAN_MASTER      numeric not null,
    FK_IBAN_ATTRIBUTE numeric not null,
    constraint PK_IBAN_ATTRIBUTE_MASTER
        primary key (OBJ_ID),
    constraint FK_IBAN_MASTER
        foreign key (FK_IBAN_MASTER)
            references NODO4_CFG.IBAN_MASTER,
    constraint FK_IBAN_ATTRIBUTE
            foreign key (FK_IBAN_ATTRIBUTE)
                references NODO4_CFG.IBAN_ATTRIBUTE
);
