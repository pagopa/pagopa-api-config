  ...
  public static Specification<Canali> filterChannelsByCodeAndBrokerDescriptionFilters(
    ...
    return (root, query, cb) -> {
      ...
      if (StringUtils.isNotEmpty(brokerCode)) {
        predicates.add(cb.and(cb.equal(canaliIntermediariPspJoin.get("idIntermediarioPsp"), brokerCode)));
      }

      if (StringUtils.isNotEmpty(channelId)) {
        predicates.add(cb.and(cb.like(cb.lower(root.get("idCanale")), "%" + channelId.toLowerCase() + "%")));
      }

      if (StringUtils.isNotEmpty(brokerDescription)) {
        predicates.add(cb.and(cb.like(cb.lower(canaliIntermediariPspJoin.get("codiceIntermediario")), "%" + brokerDescription.toLowerCase() + "%")));
      }
