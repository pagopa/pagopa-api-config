...
import it.gov.pagopa.apiconfig.model.massiveloading.CreditorInstitutionStation;
...
public class CreditorInstitutionStationVerifier
  ...
  public CreditorInstitutionStationVerifier(
      String environment,
   ...
   */
  @Override
  public boolean verifyBean(CreditorInstitutionStation creditorInstitutionStation)
    ...
    } else if (optEc.isPresent()
        && optStation.isPresent()
        && creditorInstitutionStation
            .getOperation()
            .equals(CreditorInstitutionStation.Operation.C)) {
      ...
      throw new CsvConstraintViolationException(
          String.format(
              ...
              ToStringBuilder.reflectionToString(
                  creditorInstitutionStation, ToStringStyle.NO_CLASS_NAME_STYLE),
              String.join(" # ", errors)));
    }
  ...
  private void checkIfFound(
    ...
    Long segregationCode =
        creditorInstitutionStation.getSegregationCode() != null
            ? Long.parseLong(creditorInstitutionStation.getSegregationCode())
            : null;
    Long applicationCode =
        creditorInstitutionStation.getApplicationCode() != null
            ? Long.parseLong(creditorInstitutionStation.getApplicationCode())
            : null;
    Optional<PaStazionePa> optECStation =
        paStazionePaRepository
            .findByFkPaAndFkStazione_ObjIdAndAuxDigitAndBroadcastAndSegregazioneAndProgressivo(
                optEc.getObjId(),
                optStation.getObjId(),
                creditorInstitutionStation.getAuxDigit(),
  ...
  private void checkIfExists(List<String> errors, Pa optEc, Stazioni optStation) {
    Optional<PaStazionePa> relation =
        paStazionePaRepository.findAllByFkPaAndFkStazione_ObjId(
            optEc.getObjId(), optStation.getObjId());
   ...
   * @param errors list of errors
   */
  private void checkSegregationAndApplicationCode(
      CreditorInstitutionStation creditorInstitutionStation, List<String> errors) {
    if (creditorInstitutionStation.getAuxDigit() == 1
        || creditorInstitutionStation.getAuxDigit() == 2) {
  ...
  private void checkSegregationAndApplicationCode0(
      CreditorInstitutionStation creditorInstitutionStation, List<String> errors) {
    if (creditorInstitutionStation.getApplicationCode() == null
        || creditorInstitutionStation.getApplicationCode().isBlank()
        || creditorInstitutionStation.getApplicationCode().length() != 2) {
      errors.add("Application code error: length must be 2 ciphers");
    }

    if (creditorInstitutionStation.getSegregationCode() != null
        && !creditorInstitutionStation.getSegregationCode().isBlank()
  ...
  private void checkSegregationAndApplicationCode3(
      CreditorInstitutionStation creditorInstitutionStation, List<String> errors) {
    if (creditorInstitutionStation.getApplicationCode() != null
        && !creditorInstitutionStation.getApplicationCode().isBlank()
        && creditorInstitutionStation.getApplicationCode().length() != 2) {
   ...
   */
  private Optional<Stazioni> checkStation(
      CreditorInstitutionStation creditorInstitutionStation, List<String> errors) {
    Optional<Stazioni> optStation =
        stazioniRepository.findByIdStazione(creditorInstitutionStation.getStationId());
   ...
   */
  private Optional<Pa> checkCreditorInstitution(
      CreditorInstitutionStation creditorInstitutionStation, List<String> errors) {
    Optional<Pa> optEc =
        paRepository.findByIdDominio(creditorInstitutionStation.getCreditorInstitutionId());
    if (optEc.isEmpty()) {
